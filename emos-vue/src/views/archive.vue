...
export default {
	...
	methods: {
		...
		successHandle: function(resp, file, fileList) {
			if (resp && resp.code === 200) {
				for (let one of fileList) {
					this.picList[one.response.url] = one.response.key;
				}
			} else {
		...
		removeHandle: function(file, fileList) {
			...
			let that = this;
			let key = this.picList[url];
			that.$http('cos/deleteCosFile', 'POST', { keys: [key] }, true, function(resp) {
		...
		cancel: function() {
			...
			if (Object.keys(that.picList).length > 0) {
				var keys = Object.values(that.picList);
				that.$http('cos/deleteCosFile', 'POST', { keys: keys }, true, function(resp) {
					that.picList = {};
				});
			}
			that.visible = false;
		...
		archive: function() {
			...
			let files = [];
			for (let one in that.picList) {
				files.push({
					url: one,
					key: that.picList[one]
				});
			}
			let data = {
				taskId: that.taskId,
